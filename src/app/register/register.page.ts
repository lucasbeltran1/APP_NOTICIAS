...
export class RegisterPage {
  ...
  async onRegister() {
    if (this.registerForm.valid && !this.passwordMismatch) { // Si el formulario es válido y las contraseñas coinciden
      ...
      try {
        // Asegúrate de que el método register del AuthService acepte los 3 argumentos
        ...
        const errorMessage = (error as Error).message || 'Error desconocido'; // Convertir el error a tipo Error y obtener el mensaje
        const toast = await this.toastController.create({
