...
export class WeatherService {
  ...
  // URLs base para el clima actual, pronóstico y geocodificación inversa
  private baseUrl = 'https://api.openweathermap.org/data/2.5/weather';
  private forecastUrl = 'https://api.openweathermap.org/data/2.5/forecast';
  private reverseGeocodeUrl = 'https://api.openweathermap.org/geo/1.0/reverse';
  ...
  /**
   * Método para obtener el nombre de la ciudad basado en coordenadas (geocodificación inversa).
   * @param lat - Latitud.
   * @param lon - Longitud.
   * @returns Un Observable con la respuesta del API de OpenWeather.
   */
  getCityNameByCoordinates(lat: number, lon: number): Observable<any> {
    const url = `${this.reverseGeocodeUrl}?lat=${lat}&lon=${lon}&limit=1&appid=${this.apiKey}`;
    return this.http.get<any>(url);
  }
}
